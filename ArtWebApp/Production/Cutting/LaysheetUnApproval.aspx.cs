...
{
    public partial class LaysheetUnApproval : System.Web.UI.Page
    {
        ...
        {
            Filllaysheet(int.Parse(drp_ourstyle.SelectedValue.ToString()), int.Parse(drp_fact.SelectedValue.ToString()));
        }

        protected void btn_showroll_Click(object sender, EventArgs e)
        {
            int laysheetpk = int.Parse(drp_cutRoll.SelectedValue.ToString());
        ...
        public void FillLaySheetmasterDatausingEntity(int laysheetpk)
        {
            using (ArtEntitiesnew entty = new ArtEntitiesnew())
            {
                var q = from lymstr in entty.LaySheetMasters
                        join cutorderdet in entty.CutOrderDetails on lymstr.CutOrderDet_PK equals cutorderdet.CutOrderDet_PK
                ...
                {
                    lbl_laysheetnum.Text = element.LaySheetNum.ToString().Trim();
                    lbl_manualcutnum.Text = element.LayCutNum.ToString().Trim();
   ...
   ExcessOrShort = 0;
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                AYard += Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "AYard"));
                BalToCut += Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "BalToCut"));
                NoOfPlies += Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "NoOfPlies"));
                FabUtilized += Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "FabUtilized"));
                ExcessOrShort += Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "ExcessOrShort"));
            ...
            if (e.Row.RowType == DataControlRowType.Footer)
            {
                ...
                Label lbl_footAYard = (Label)e.Row.FindControl("lbl_footAYard");
                Label lbl_footFabUtilized = (Label)e.Row.FindControl("lbl_footFabUtilized");
                Label lbl_footExcessOrShort = (Label)e.Row.FindControl("lbl_footExcessOrShort");
                Label lbl_footBalToCut = (Label)e.Row.FindControl("lbl_footBalToCut");
                Label lbl_footNoOfPlies = (Label)e.Row.FindControl("lbl_footNoOfPlies");


                lbl_footAYard.Text = AYard.ToString();
                lbl_footFabUtilized.Text = FabUtilized.ToString();
                lbl_footExcessOrShort.Text = ExcessOrShort.ToString();
                lbl_footBalToCut.Text = BalToCut.ToString();
                lbl_footNoOfPlies.Text = NoOfPlies.ToString();
            }
        }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            LaySheetfunction.unaprovallaysheet(int.Parse(drp_cutRoll.SelectedValue.ToString()));
