<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ASQBuyerPOReport.aspx.cs" EnableEventValidation="false" Inherits="ArtWebApp.Reports.MerchandiserReport.ASQBuyerPOReport" %>
...
<iframe id="txtArea1" style="display:none"></iframe>
         ...
         <asp:SqlDataSource ID="FACTORYDATASOURCE" runat="server" ConnectionString="<%$ ConnectionStrings:ArtConnectionString %>" SelectCommand="SELECT [Location_PK], [LocationName] FROM [LocationMaster] WHERE ([LocType] = @LocType)">
                            ...
                            <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:ArtConnectionString2 %>" 
                                SelectCommand="SELECT ASQ, PoPacknum, BuyerPO, PoPackId, OurStyleID, OurStyle, BuyerStyle, AtcId, IsCutable, IsPackable, DeliveryDate, SeasonName, ChannelName, BuyerDestination, AtcNum, CategoryName, CategoryID, ChannelID, BuyerDestination_PK, Season_PK, BuyerID, BuyerName, PackingInstruction, ExpectedLocation_PK, LocationName, HandoverDate FROM (SELECT TOP (100) PERCENT PoPackMaster.PoPacknum + ' / ' + PoPackMaster.BuyerPO AS ASQ, PoPackMaster.PoPacknum, PoPackMaster.BuyerPO, PoPackMaster.PoPackId, POPackDetails.OurStyleID, AtcDetails.OurStyle, AtcDetails.BuyerStyle, PoPackMaster.AtcId, MAX(POPackDetails.IsCutable) AS IsCutable, POPackDetails.IsPackable, CAST(PoPackMaster.DeliveryDate AS date) AS DeliveryDate, PoPackMaster.SeasonName, ISNULL((SELECT DISTINCT LocationMaster.LocationName FROM ASQAllocationMaster INNER JOIN LocationMaster ON ASQAllocationMaster.Locaion_PK = LocationMaster.Location_PK WHERE (ASQAllocationMaster.PoPackId = PoPackMaster.PoPackId) AND (ASQAllocationMaster.Qty &gt; 0) AND (ASQAllocationMaster.OurStyleId = POPackDetails.OurStyleID)), LocationMaster_1.LocationName) AS LocationName, ChannelMaster.ChannelName, BuyerDestinationMaster.BuyerDestination, AtcMaster.AtcNum, GarmentCategory.CategoryName, GarmentCategory.CategoryID, ChannelMaster.ChannelID, BuyerDestinationMaster.BuyerDestination_PK, SeasonMaster.Season_PK, BuyerMaster.BuyerID, BuyerMaster.BuyerName, PoPackMaster.PackingInstruction, PoPackMaster.ExpectedLocation_PK, PoPackMaster.HandoverDate FROM PoPackMaster INNER JOIN POPackDetails ON PoPackMaster.PoPackId = POPackDetails.POPackId INNER JOIN AtcDetails ON POPackDetails.OurStyleID = AtcDetails.OurStyleID INNER JOIN ChannelMaster ON PoPackMaster.ChannelID = ChannelMaster.ChannelID INNER JOIN BuyerDestinationMaster ON PoPackMaster.BuyerDestination_PK = BuyerDestinationMaster.BuyerDestination_PK INNER JOIN AtcMaster ON AtcDetails.AtcId = AtcMaster.AtcId INNER JOIN GarmentCategory ON AtcDetails.CategoryID = GarmentCategory.CategoryID INNER JOIN SeasonMaster ON PoPackMaster.SeasonName = SeasonMaster.SeasonName INNER JOIN BuyerMaster ON AtcMaster.Buyer_ID = BuyerMaster.BuyerID LEFT OUTER JOIN LocationMaster AS LocationMaster_1 ON PoPackMaster.ExpectedLocation_PK = LocationMaster_1.Location_PK GROUP BY PoPackMaster.PoPacknum + ' / ' + PoPackMaster.BuyerPO, PoPackMaster.PoPacknum, PoPackMaster.BuyerPO, PoPackMaster.PoPackId, POPackDetails.OurStyleID, AtcDetails.OurStyle, AtcDetails.BuyerStyle, PoPackMaster.AtcId, POPackDetails.IsPackable, PoPackMaster.DeliveryDate, PoPackMaster.SeasonName, ChannelMaster.ChannelName, BuyerDestinationMaster.BuyerDestination, AtcMaster.AtcNum, GarmentCategory.CategoryName, GarmentCategory.CategoryID, ChannelMaster.ChannelID, BuyerDestinationMaster.BuyerDestination_PK, SeasonMaster.Season_PK, BuyerMaster.BuyerID, BuyerMaster.BuyerName, PoPackMaster.PackingInstruction, PoPackMaster.ExpectedLocation_PK, LocationMaster_1.LocationName, PoPackMaster.HandoverDate HAVING (PoPackMaster.AtcId = @Param1) AND (SUM(POPackDetails.RevisedQty) &gt; 0) ORDER BY PoPackMaster.PoPackId) AS tttt">
                                <SelectParameters>
