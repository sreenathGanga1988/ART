...
";
        ...
        /// <param name="laysheetPK"></param>
        /// <returns></returns>
        public static DataTable LaySheetSummary(int atcid)
        {

            using (SqlCommand cmd = new SqlCommand())
            {
                cmd.CommandText = "LaySheetSummary_SP";
                cmd.CommandType = CommandType.StoredProcedure;
...
FROM            (SELECT        CutOrderMaster.CutID, CutOrderMaster.Cut_NO, AtcMaster.AtcNum, AtcDetails.OurStyle, CutOrderMaster.Color, CutOrderMaster.CutQty, CutOrderMaster.FabQty, CutOrderMaster.CutWidth, 
                         CutOrderMaster.Shrinkage, CutOrderMaster.MarkerType, CutOrderMaster.PaternName,
                             ...
                             ((SELECT        SUM(LaySheetDetails.NoOfPlies * CutOrderSizeDetails.Ratio) AS Alreadylayed
                                 ...
                                 GROUP BY CutOrderDetails.CutID), 0) AS layed, CutOrderMaster.CutPlan_Pk, LocationMaster.LocationName, CutOrderMaster.CutOrderDate, AtcMaster.IsCuttingCompleted, CutOrderMaster.IsClosed
...
WHERE        (CutOrderMaster.CutPlan_Pk IS NOT NULL) AND (CutOrderMaster.CutOrderDate > CONVERT(DATETIME, '2017-05-15 00:00:00', 102)) AND (AtcMaster.IsCuttingCompleted = N'N') AND 
                         (CutOrderMaster.IsClosed = N'N')) AS tt
